...
async function completeMode(mode) {
  for (const cat of selectedCategories) {
    if (mode === 'quiz' || mode === 'cards') {
      ...
      await sendProgress(cat, 'quiz');
      await sendProgress(cat, 'cards');
    } else {
      ...
      await sendProgress(
        cat,
        'test',
        categoryProgress[cat].testScore,
        categoryProgress[cat].testMaxScore
      );
    }
  }
...
async function sendProgress(category, mode, score = null, maxScore = null) {
  try {
